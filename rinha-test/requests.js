...
export async function getPPPaymentsSummary(service, from, to) {
  ...
  console.error(
    `Não foi possível obter resposta de '/admin/payments-summary?from=${from}&to=${to}' para ${service} (HTTP ${response.status})`,
  );
...
export async function resetBackendDatabase() {
  try {
    ...
    console.info(
      'Seu backend provavelmente não possui um endpoint para resetar o banco. Isso não é um problem.',
      error.message,
    );
  }
}
...
export async function getBackendPaymentsSummary(from, to) {
  ...
  console.error(
    `Não foi possível obter resposta de '/payments-summary?from=${from}&to=${to}' para o backend (HTTP ${response.status})`,
  );
